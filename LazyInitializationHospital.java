class HospitalOperation {
	...
	getInstanceSynchronizedBlockWay(){
		if (_instanceForDoubleCheckLocking == null)
			synchronized (HospitalOperation.class){
				if (_instanceForDoubleCheckLocking == null)
					_instanceForDoubleCheckLocking= new HospitalOperation();
			}
	...
	public synchronized void operation(String aName){
		if (empty == true) {
			patientName = aName;
			System.out.println("Operation can be done "+ "get ready patient "+ patientName);
			empty = false;
		}

		else {
			System.out.println("Sorry " + aName+ " Operation Theatre is busy with Surgery of "+ patientName);
		}
	}
}
public class  LazyInitializationHospital {
	public static void main(String args[]){
		Thread t1 = new Thread(new Runnable() {
			public void run(){
			HospitalOperation i1= HospitalOperation.getInstanceSynchronizedWay();
		...
		Thread t2 = new Thread(new Runnable() {
			public void run(){
			HospitalOperation i2= HospitalOperation.getInstanceSynchronizedWay();
		...
		public void run(){
		HospitalOperation i1= HospitalOperation.getInstanceSynchronizedBlockWay();
		...
		public void run(){
		HospitalOperation i2= HospitalOperation.getInstanceSynchronizedBlockWay();
